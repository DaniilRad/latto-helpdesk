...
import { Plus, Search, Inbox, Bookmark, BookmarkPlus, X } from "lucide-react";
import { Button, Input, Select, Avatar, Dialog, Badge } from "../ds";
import { PageHeader, TableCard, HoverRow, thStyle, tdStyle, EmptyState, StatCard, SortableTh, useSort, Field } from "../components/bits.jsx";
import { PriorityBadge, StatusBadge, SlaBadge } from "../components/ticketBits.jsx";
import { TicketDialog } from "../components/TicketDialog.jsx";
import { TICKET_STATUS, OPEN_STATUSES, slaState, relTime } from "../lib/meta.js";
...
export function Tickets() {
  ...
  const applySearch = (s) => { setQ(s.q || ""); setStatus(s.status || "open"); setAssignee(s.assignee || "all"); };
  const saveCurrent = () => {
    if (!searchName.trim()) return;
    addSavedSearch({ name: searchName.trim(), q, status, assignee });
    setSearchName(""); setSaving(false);
  };
  ...
  const filtered = visible.filter((t) => {
    ...
    const hay = [t.number, t.title, t.description, userById(t.requesterId)?.displayName || ""].join(" ").toLowerCase();
  ...
  return (
    <>
      <PageHeader eyebrow="CORE TICKETING" title={seeAll ? "Tickets" : "My tickets"}
        actions={<Button iconLeft={<Plus size={16} />} onClick={() => setCreating(true)}>New ticket</Button>} />

      {seeAll && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 16 }}>
          <StatCard label="Open" value={open.length} />
          <StatCard label="SLA at risk" value={atRisk.length} sub={atRisk.length ? "act now" : "all green"}
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
        ...
        <span style={{ flex: 1 }} />
        <Button variant="ghost" size="sm" iconLeft={<BookmarkPlus size={14} />} disabled={isDefault}
          onClick={() => setSaving(true)}>Save search</Button>
      </div>
      ...
      {savedSearches.length > 0 && (
        <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap", alignItems: "center" }}>
          ...
          {savedSearches.map((s) => (
            <span key={s.id} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "4px 6px 4px 12px",
              ...
              {s.name}
              <button onClick={(e) => { e.stopPropagation(); deleteSavedSearch(s.id); }}
                style={{ display: "inline-flex", border: "none", background: "transparent", color: "var(--text-3)",
                  cursor: "pointer", padding: 2, borderRadius: 999 }} title="Delete saved search">
                <X size={12} />
              </button>
            </span>
          ))}
      ...
      {filtered.length === 0 ? (
        <EmptyState icon={<Inbox size={28} />} text="No tickets match. That's either great news or a filter problem."
          action={<Button size="sm" variant="secondary" onClick={() => setCreating(true)}>New ticket</Button>} />
      ) : (
        <TableCard>
          <thead><tr>
            <SortableTh k="number" sort={sort}>#</SortableTh>
            <SortableTh k="priority" sort={sort}>Priority</SortableTh>
            <SortableTh k="title" sort={sort}>Title</SortableTh>
            <SortableTh k="status" sort={sort}>Status</SortableTh>
            <th style={thStyle}>SLA</th>
            <SortableTh k="requester" sort={sort}>Requester</SortableTh>
            {seeAll && <SortableTh k="assignee" sort={sort}>Assignee</SortableTh>}
            <SortableTh k="updated" sort={sort} align="right">Updated</SortableTh>
          </tr></thead>
          <tbody>
            {sort.sorted.map((t) => {
              ...
              return (
                <HoverRow key={t.id} onClick={() => nav(`/tickets/${t.id}`)}>
                  ...
                  <td style={{ ...tdStyle, fontFamily: "var(--font-sans)", color: "var(--text-1)", whiteSpace: "normal", minWidth: 220 }}>
                    {t.title}
                  </td>
                  <td style={tdStyle}><StatusBadge status={t.status} /></td>
                  <td style={tdStyle}><SlaBadge ticket={t} /></td>
                  <td style={{ ...tdStyle, fontFamily: "var(--font-sans)" }}>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
                      <Avatar name={req?.displayName || "?"} size={22} />{req?.displayName || "—"}
                    </span>
                  </td>
                  {seeAll && (
                    <td style={{ ...tdStyle, fontFamily: "var(--font-sans)", color: asg ? "var(--text-1)" : "var(--text-3)" }}>
                      {asg?.displayName || "—"}
                    </td>
                  )}
      ...
      <Dialog open={saving} onClose={() => setSaving(false)} title="Save this search"
        ...
        <Field label="Name">
          <Input value={searchName} autoFocus onChange={(e) => setSearchName(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && saveCurrent()} placeholder="e.g. My unassigned P1s" />
        </Field>
