...
import { Badge, Button, Dialog, Input, Select } from "../ds";
import { CONTRACT_TYPES, daysUntil, formatDate } from "../lib/meta.js";
...
export function Contracts() {
  ...
  const submit = () => {
    if (!form.name?.trim()) {
      toast("danger", "Name required", "Give the contract or certificate a name.");
      return;
    }
    addContract({
      name: form.name.trim(),
  ...
  return (
    <>
      ...
      <TableCard>
        <thead>
          <tr>
            ...
            <SortableTh k="vendor" sort={sort}>
              Vendor
            </SortableTh>
            <SortableTh k="end" sort={sort}>
              Ends
            </SortableTh>
            <SortableTh k="cost" sort={sort}>
              Cost
            </SortableTh>
            <th
              style={{
      ...
      <Dialog
        ...
        title="Add contract / certificate"
        actions={[
      ...
      >
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <Field label="Number / CN">
            <Input value={form.number || ""} onChange={set("number")} placeholder="*.latto.io" />
          </Field>
          <Field label="Cost">
            <Input value={form.cost || ""} onChange={set("cost")} placeholder="€ 180 / y" />
          </Field>
          <Field label="Starts">
