...
import { Plus, Search, Boxes, Layers } from "lucide-react";
import { Button, Input, Select, Badge, Dialog, Avatar } from "../ds";
import { PageHeader, TableCard, HoverRow, tdStyle, EmptyState, StatCard, SortableTh, useSort, Field } from "../components/bits.jsx";
import { PriorityBadge } from "../components/ticketBits.jsx";
import { Textarea } from "../components/ticketBits.jsx";
import {
  PROBLEM_STATUS, PROBLEM_OPEN_STATUSES, LEVELS, computePriority, relTime,
} from "../lib/meta.js";
import { useStore } from "../lib/store.jsx";

function ProblemStatusBadge({ status }) {
  const s = PROBLEM_STATUS[status] || PROBLEM_STATUS.open;
  return <Badge tone={s.tone} dot pulse={status === "investigating"}>{s.label}</Badge>;
}
...
export function Problems() {
  ...
  return (
    <>
      <PageHeader eyebrow="CORE TICKETING" title="Problems"
        actions={<Button iconLeft={<Plus size={16} />} onClick={() => setCreating(true)}>New problem</Button>}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginBottom: 16 }}>
        ...
        <StatCard label="Known errors" value={knownErrors.length} sub={knownErrors.length ? "with workaround" : ""}
      ...
      {sort.sorted.length === 0 ? (
        <EmptyState icon={<Boxes size={28} />} text="No problems match. Nothing systemic on fire right now."
          action={<Button size="sm" variant="secondary" onClick={() => setCreating(true)}>New problem</Button>} />
      ) : (
        <TableCard>
          <thead><tr>
            ...
            <SortableTh k="tickets" sort={sort}>Tickets</SortableTh>
            ...
            {sort.sorted.map((p) => {
              ...
              return (
                <HoverRow key={p.id} onClick={() => nav(`/problems/${p.id}`)}>
                  ...
                  <td style={tdStyle}><PriorityBadge priority={computePriority(p.impact, p.urgency)} /></td>
                  <td style={{ ...tdStyle, fontFamily: "var(--font-sans)", color: "var(--text-1)", whiteSpace: "normal", minWidth: 240 }}>
                    {p.title}
                    {asg && (
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 6, marginLeft: 8,
                        fontSize: 11, color: "var(--text-3)", verticalAlign: "middle" }}>
                        <Avatar name={asg.displayName} size={18} />{asg.displayName}
                      </span>
                    )}
                  </td>
                  <td style={tdStyle}><ProblemStatusBadge status={p.status} /></td>
                  <td style={tdStyle}>
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                      <Layers size={13} style={{ color: "var(--text-3)" }} />{linkCount(p.id)}
                    </span>
                  </td>
      ...
      <Dialog open={creating} onClose={() => setCreating(false)} title="New problem"
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <Field label="Impact">
            <Select value={form.impact || "medium"} onChange={set("impact")} style={{ width: "100%" }}
              options={Object.entries(LEVELS).map(([v, l]) => ({ value: v, label: l.label }))} />
          </Field>
          <Field label="Urgency">
            <Select value={form.urgency || "medium"} onChange={set("urgency")} style={{ width: "100%" }}
              options={Object.entries(LEVELS).map(([v, l]) => ({ value: v, label: l.label }))} />
          </Field>
        </div>
      </Dialog>
