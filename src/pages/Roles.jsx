import { ArrowDown, ArrowUp, Building2, GitBranch, Plus, ShieldCheck, Trash2, Users2, X } from "lucide-react";
import React from "react";
import { Eyebrow, Field, HoverRow, PageHeader, TableCard, tdStyle, thStyle } from "../components/bits.jsx";
import { Avatar, Badge, Button, Card, Dialog, Input, Select, Switch, Tabs } from "../ds";
import { PERMISSIONS, ROLES, RULE_TYPES } from "../lib/rbac.js";
              ...
              {PERMISSIONS.map((p, i) => (
                <div
                  ...
                  style={{
                    ...
                    padding: "7px 0",
                    borderBottom: i < PERMISSIONS.length - 1 ? "1px solid var(--border-faint)" : "none",
                  }}
                >
                  <span style={{ fontSize: 13, color: "var(--text-2)" }}>{p.label}</span>
                  <Switch
                    size="sm"
                    checked={Boolean(profiles[role]?.[p.key])}
                    disabled={role === "admin"}
...
function RulesTab() {
  ...
  const matchCount = (ruleId) => users.filter((u) => u.status !== "disabled" && roleOf(u).ruleId === ruleId).length;
      ...
      <TableCard>
        ...
        <tbody>
            ...
            <HoverRow key={r.id}>
              ...
              <td style={{ ...tdStyle, textAlign: "right" }}>
                <span style={{ display: "inline-flex", gap: 2 }}>
                  <Button variant="ghost" size="sm" disabled={i === 0} onClick={() => moveRule(r.id, -1)}>
                    <ArrowUp size={14} />
                  </Button>
                  <Button variant="ghost" size="sm" disabled={i === rules.length - 1} onClick={() => moveRule(r.id, 1)}>
                    <ArrowDown size={14} />
                  </Button>
          ...
          <tr>
            ...
            <td style={tdStyle}>—</td>
            <td style={tdStyle}>
              <Badge tone="success">{ROLES["end-user"].label}</Badge>
            </td>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <Field label="Value" span={2}>
            <Input
              ...
              onChange={set("value")}
              placeholder={
                form.type === "attribute" ? "employeeType=External" : form.type === "ou" ? "OU=IT" : "IT-Support"
              }
            />
          </Field>
...
function GroupsTab() {
  ...
  const submit = () => {
    if (!form.name.trim()) {
      toast("danger", "Name required", "Give the team a name.");
      return;
    }
                ...
                {members.map((m) => (
                  <div key={m.id} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <Button
                      ...
                      size="sm"
                      title="Remove"
                      onClick={() => saveGroup(g.id, { members: g.members.filter((id) => id !== m.id) })}
                    >
                      <X size={13} />
                    </Button>
                  </div>
...
function EntitiesTab() {
  ...
  const openEdit = (e) => {
    setForm({ name: e.name, description: e.description || "" });
    setDialog(e);
  };
  const submit = () => {
    if (!form.name.trim()) {
      toast("danger", "Name required", "Give the entity a name.");
      return;
    }
  ...
  return (
    <>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
        <p style={{ margin: 0, fontSize: 13, color: "var(--text-3)", flex: 1 }}>
          Multi-tenancy boundaries. Staff can switch entities from the topbar; end-users and externals are hard-scoped
          to theirs.
        </p>
        <Button size="sm" iconLeft={<Plus size={15} />} onClick={openNew}>
          Add entity
        </Button>
      </div>
        ...
        <div style={{ display: "grid", gap: 14 }}>
          <Field label="Name" required>
            <Input
              ...
              autoFocus
              onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
              placeholder="e.g. Subsidiary North"
            />
          </Field>
          <Field label="Description">
