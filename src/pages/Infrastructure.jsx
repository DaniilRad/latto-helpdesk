...
import { Badge, Button, Card, Dialog, Input, Select, Tabs, Tag } from "../ds";
import { DEVICE_TYPES, daysUntil, formatDate, relTime } from "../lib/meta.js";
  ...
  return (
    <>
          ...
          {databases.map((db) => {
            ...
            return (
              <HoverRow key={db.id}>
                ...
                <td style={tdStyle}>
                  {host ? (
                    <Link to={`/devices/${host.id}`} style={{ color: "var(--accent-text)", textDecoration: "none" }}>
                      {host.name}
                    </Link>
                  ) : (
                    "—"
      ...
      <AddDialog
        ...
        onSubmit={(f) => {
          if (!f.name?.trim()) return false;
          addDatabase({ ...f, sizeGb: Number(f.sizeGb) || 0 });
          return true;
        }}
      />
    </>
  ...
  return (
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 14 }}>
        {datacenters.map((dc) => {
          const hosted = devices.filter(
            (d) => d.location?.toLowerCase().includes("server room") && dc.name.toLowerCase().includes("server room"),
          );
          return (
            <div
              key={dc.id}
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                ...
                {canEdit && (
                  <Button variant="ghost" size="sm" onClick={() => deleteDatacenter(dc.id)}>
                    <Trash2 size={14} />
                  </Button>
                )}
              </div>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--text-3)", marginBottom: 10 }}>
                {dc.location}
              </div>
...
function UnmanagedTab() {
  ...
  if (unmanaged.length === 0) {
    return (
      <EmptyState icon={<Radar size={28} />} text="No unmanaged devices on the network. Everything's accounted for." />
    );
  }
  return (
    <>
              ...
              <td style={{ ...tdStyle, textAlign: "right" }}>
                {canEdit && (
                  <span style={{ display: "inline-flex", gap: 4 }}>
                    ...
                    <Button variant="ghost" size="sm" onClick={() => dismissUnmanaged(u.id)}>
                      <X size={14} />
                    </Button>
                  </span>
                )}
      ...
      <Dialog
        ...
        description="It moves into the device inventory as in-stock."
        actions={[
          { label: "Cancel", variant: "ghost", onClick: () => setApproving(null) },
          {
            ...
            variant: "primary",
            onClick: () => {
              approveUnmanaged(approving.id, type);
              setApproving(null);
            },
          },
        ]}
    ...
    >
      ...
      <span
        style={{
          ...
          fontFamily: accent ? "var(--font-sans)" : "var(--font-mono)",
        }}
      >
        {label}
      </span>
        ...
        {datacenters.map((dc) => {
          ...
          return (
            <div key={dc.id}>
              ...
              />
              {dcOpen &&
                (dc.racks?.length ? dc.racks : ["—"]).map((rack) => {
    ...
    <>
      ...
      <Tabs
        ...
        tabs={[
          ...
          {
            value: "unmanaged",
            label: `Unmanaged${unmanaged.length ? ` (${unmanaged.length})` : ""}`,
            icon: <Radar size={15} />,
          },
        ]}
      />
