...
export function Portal() {
  ...
  const hits = needle
    ? publicKb
        .filter((a) => [a.title, a.body, (a.tags || []).join(" ")].join(" ").toLowerCase().includes(needle))
        .slice(0, 5)
    : publicKb.slice(0, 4);
      ...
      <div style={{ maxWidth: 880, margin: "0 auto" }}>
        <div style={{ textAlign: "center", padding: "26px 0 22px" }}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "center", alignItems: "center" }}>
            <Input
              ...
              wrapStyle={{ width: 380 }}
            />
            <Button size="lg" iconLeft={<Plus size={17} />} onClick={() => setCreating(true)}>
              New ticket
            </Button>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, alignItems: "start" }}>
          <Card padding="0">
            ...
            >
              <Eyebrow>
                <LifeBuoy size={11} style={{ verticalAlign: "-1px", marginRight: 6 }} />
                MY TICKETS · {open.length} OPEN
              </Eyebrow>
              <Button variant="ghost" size="sm" iconRight={<ArrowRight size={14} />} onClick={() => nav("/tickets")}>
                All
              </Button>
            </div>
            {mine.length === 0 ? (
            ...
            {awaitingApproval.length > 0 && (
              <div style={{ padding: "10px 18px 14px", borderTop: "1px solid var(--border)" }}>
                <Badge tone="success" dot>
                  {awaitingApproval.length} resolved — please confirm
                </Badge>
              </div>
            )}
          </Card>
          ...
          <Card padding="0">
            ...
            >
              <Eyebrow>
                <BookOpen size={11} style={{ verticalAlign: "-1px", marginRight: 6 }} />
                {needle ? "SEARCH RESULTS" : "POPULAR ARTICLES"}
              </Eyebrow>
              <Button variant="ghost" size="sm" iconRight={<ArrowRight size={14} />} onClick={() => nav("/kb")}>
                Browse
              </Button>
            </div>
            {hits.length === 0 ? (
              ...
              hits.map((a, i) => (
                ...
                >
                  ...
                  <span
                    style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-3)", flex: "0 0 auto" }}
                  >
                    {a.category}
                  </span>
                </div>
