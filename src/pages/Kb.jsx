...
import { Plus, Search, BookOpen, ArrowLeft, Pencil, Trash2, Lock, Globe } from "lucide-react";
import { Card, Badge, Button, Input, Select, Dialog, Tag } from "../ds";
import { PageHeader, Eyebrow, EmptyState, Field } from "../components/bits.jsx";
...
function ArticleDialog({ open, onClose, article }) {
  ...
  const submit = () => {
    ...
    const payload = {
      title: form.title.trim(), category: form.category.trim() || "General",
      visibility: form.visibility, body: form.body,
      tags: form.tagsText.split(",").map((t) => t.trim()).filter(Boolean),
    };
    if (editing) saveArticle(article.id, payload);
    else { const id = addArticle(payload, persona.id); nav(`/kb/${id}`); }
    onClose();
  };
    ...
    <Dialog open={open} onClose={onClose} width={680}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <Field label="Title" span={2}>
          <Input value={form.title || ""} onChange={set("title")} placeholder="Set up VPN on a company notebook" autoFocus />
        </Field>
        <Field label="Category">
          <Input value={form.category || ""} onChange={set("category")} placeholder="Network" />
        </Field>
        <Field label="Visibility">
          <Select value={form.visibility} onChange={set("visibility")} style={{ width: "100%" }}
            options={[{ value: "public", label: "Public — self-service" },
        ...
        <Field label="Body" span={2}>
          <Textarea value={form.body || ""} onChange={set("body")} style={{ minHeight: 220, fontFamily: "var(--font-mono)", fontSize: 13 }} />
        </Field>
      </div>
    </Dialog>
  );
...
export function KbList() {
  ...
  return (
    <>
      <PageHeader eyebrow="KNOWLEDGE BASE" title="Knowledge base"
        actions={hasPerm("kb.write") && <Button iconLeft={<Plus size={16} />} onClick={() => setCreating(true)}>New article</Button>} />

      <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
        <Input placeholder="Search articles…" icon={<Search size={16} />} value={q}
          onChange={(e) => setQ(e.target.value)} wrapStyle={{ width: 280 }} />
            ...
            <Card key={a.id} interactive onClick={() => nav(`/kb/${a.id}`)} padding="18px">
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                <Badge tone={a.visibility === "internal" ? "warning" : "success"}>
                  {a.visibility === "internal" ? <Lock size={9} style={{ marginRight: 4 }} /> : <Globe size={9} style={{ marginRight: 4 }} />}
                  {a.visibility}
                </Badge>
                <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-3)" }}>{a.category}</span>
              </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12 }}>
                {(a.tags || []).slice(0, 3).map((t) => <Tag key={t} style={{ height: "1.4rem", fontSize: 12 }}>{t}</Tag>)}
...
export function KbArticle() {
  ...
  if (!allowed) {
    return (
      <>
        <Button variant="ghost" size="sm" iconLeft={<ArrowLeft size={15} />} onClick={() => nav("/kb")}>Knowledge base</Button>
        <p style={{ color: "var(--text-2)" }}>{article ? "This article is internal." : "This article doesn't exist anymore."}</p>
      </>
    );
  }
  ...
  return (
    <>
      <Button variant="ghost" size="sm" iconLeft={<ArrowLeft size={15} />}
        onClick={() => nav("/kb")} style={{ marginBottom: 10 }}>Knowledge base</Button>

      <PageHeader eyebrow={`${article.category.toUpperCase()} · ${article.visibility.toUpperCase()}`} title={article.title}
        actions={hasPerm("kb.write") && (
          <>
            <Button variant="secondary" iconLeft={<Pencil size={15} />} onClick={() => setEditing(true)}>Edit</Button>
            <Button variant="danger" iconLeft={<Trash2 size={15} />} onClick={() => setConfirming(true)}>Delete</Button>
          </>
        )}>
      ...
      <Card style={{ maxWidth: 760 }}>
        ...
        {(article.tags || []).length > 0 && (
          <div style={{ display: "flex", gap: 8, marginTop: 20, paddingTop: 16, borderTop: "1px solid var(--border-faint)" }}>
      ...
      <Dialog open={confirming} onClose={() => setConfirming(false)}
        title="Delete this article?" description="This can't be undone."
        actions={[
          { label: "Cancel", variant: "ghost", onClick: () => setConfirming(false) },
          { label: "Delete", variant: "danger", onClick: () => { deleteArticle(article.id); nav("/kb"); } },
        ]} />
    </>
  );
}
