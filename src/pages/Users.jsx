...
import { EmptyState, HoverRow, PageHeader, SortableTh, TableCard, tdStyle, useSort } from "../components/bits.jsx";
import { UserDialog } from "../components/UserDialog.jsx";
import { Avatar, Badge, Button, Input, Select } from "../ds";
import { relTime, USER_STATUS } from "../lib/meta.js";
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
        <Input
          placeholder="Search name, sam, email…"
      ...
      {filtered.length === 0 ? (
        <EmptyState
          icon={<UserX size={28} />}
          text="No users match. Adjust the filters or add one."
          action={
        ...
        <TableCard>
          <thead>
            <tr>
              ...
              <SortableTh k="account" sort={sort}>
                Account
              </SortableTh>
              <SortableTh k="department" sort={sort}>
                Department
              </SortableTh>
              ...
              <SortableTh k="devices" sort={sort}>
                Devices
              </SortableTh>
              <SortableTh k="logon" sort={sort} align="right">
                Last logon
              </SortableTh>
            </tr>
          </thead>
            ...
            {sort.sorted.map((u) => {
              ...
              return (
                <HoverRow key={u.id} onClick={() => nav(`/users/${u.id}`)}>
                  ...
                  <td style={tdStyle}>
                    <Badge tone={s.tone} dot pulse={u.status === "locked"}>
                      {s.label}
                    </Badge>
                  </td>
