...
function SoftwareTab() {
  ...
  const getters = React.useMemo(() => ({
    name: (s) => s.name, publisher: (s) => s.publisher, version: (s) => s.version,
    category: (s) => s.category, installs: (s) => (s.installs || []).length,
  }), []);
    ...
    <>
      {canEdit && (
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
          <Button size="sm" iconLeft={<Plus size={15} />} onClick={() => setAdding(true)}>Add software</Button>
        </div>
      )}
      <TableCard>
        <thead><tr>
          <SortableTh k="name" sort={sort}>Name</SortableTh>
          <SortableTh k="publisher" sort={sort}>Publisher</SortableTh>
          <SortableTh k="version" sort={sort}>Version</SortableTh>
          <SortableTh k="category" sort={sort}>Category</SortableTh>
          <SortableTh k="installs" sort={sort}>Installs</SortableTh>
            ...
            <HoverRow key={s.id}>
              ...
              <td style={tdStyle}>
                ...
                <span style={{ color: "var(--text-3)" }}>
                  {" "}· {(s.installs || []).slice(0, 3).map((id) => devices.find((d) => d.id === id)?.name).filter(Boolean).join(", ")}
                  {(s.installs || []).length > 3 ? "…" : ""}
                </span>
              </td>
              {canEdit && (
                <td style={{ ...tdStyle, textAlign: "right" }}>
                  <Button variant="ghost" size="sm" onClick={() => deleteSoftware(s.id)}><Trash2 size={14} /></Button>
                </td>
              )}
            </HoverRow>
          ))}
      ...
      <Dialog open={adding} onClose={() => setAdding(false)} title="Add software"
        actions={[
...
function LicensesTab() {
  ...
  const getters = React.useMemo(() => ({
    ...
    key: (l) => l.key, seats: (l) => l.used / Math.max(1, l.seats), expires: (l) => l.expires,
  ...
  return (
    <>
      {canEdit && (
        <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
          <Button size="sm" iconLeft={<Plus size={15} />} onClick={() => setAdding(true)}>Add license</Button>
        </div>
      )}
      <TableCard>
        <thead><tr>
          <SortableTh k="name" sort={sort}>License</SortableTh>
          <SortableTh k="software" sort={sort}>Software</SortableTh>
          <SortableTh k="key" sort={sort}>Key / tenant</SortableTh>
          <SortableTh k="seats" sort={sort}>Seats</SortableTh>
          <SortableTh k="expires" sort={sort}>Expires</SortableTh>
          {canEdit && <th style={{ ...thStyle, textAlign: "right" }} />}
        </tr></thead>
        <tbody>
          ...
          {sort.sorted.map((l) => {
            ...
            return (
              <HoverRow key={l.id}>
                <td style={{ ...tdStyle, fontFamily: "var(--font-sans)", color: "var(--text-1)", fontWeight: 500 }}>{l.name}</td>
                <td style={{ ...tdStyle, fontFamily: "var(--font-sans)" }}>{sw?.name || "—"}</td>
                <td style={tdStyle}>{l.key || "—"}</td>
                <td style={tdStyle}>
                  <Badge tone={seatTone}>{l.used}/{l.seats}</Badge>
                </td>
                <td style={tdStyle}>
                  {l.expires ? (
                    <span style={{ display: "inline-flex", gap: 8, alignItems: "center" }}>
                      {formatDate(l.expires)}
                      {days != null && days <= 60 && <Badge tone={days < 0 ? "danger" : "warning"}>{days < 0 ? "expired" : `${days}d`}</Badge>}
                    </span>
                  ) : "perpetual"}
                </td>
                {canEdit && (
                  <td style={{ ...tdStyle, textAlign: "right" }}>
                    <Button variant="ghost" size="sm" onClick={() => deleteLicense(l.id)}><Trash2 size={14} /></Button>
                  </td>
                )}
              </HoverRow>
            );
      ...
      <Dialog open={adding} onClose={() => setAdding(false)} title="Add license"
        actions={[
          { label: "Cancel", variant: "ghost", onClick: () => setAdding(false) },
          { label: "Add", variant: "primary", onClick: submit },
        ]}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <Field label="Key / tenant"><Input value={form.key || ""} onChange={set("key")} /></Field>
          <Field label="Expires"><Input type="date" value={form.expires || ""} onChange={set("expires")} /></Field>
          <Field label="Seats"><Input type="number" value={form.seats || ""} onChange={set("seats")} /></Field>
          <Field label="Used"><Input type="number" value={form.used || ""} onChange={set("used")} /></Field>
        </div>
      </Dialog>
    </>
  );
