...
import { TICKET_CATEGORIES, LEVELS, computePriority, PRIORITY } from "../lib/meta.js";
...
export function TicketDialog({ open, onClose, onCreated }) {
  ...
  React.useEffect(() => {
    if (open) {
      ...
      setForm({
        ...
        requesterId: persona.id, assigneeId: "", assetId: "",
      });
    }
  }, [open, persona]);
  ...
  const submit = () => {
    ...
    const id = addTicket({
      ...
      requesterId: form.requesterId, assigneeId: form.assigneeId || null,
  ...
  return (
    <Dialog open={open} onClose={onClose} width={620} title="New ticket"
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        ...
        {full ? (
          <Field label="Requester">
            <Select value={form.requesterId} onChange={set("requesterId")} style={{ width: "100%" }}
          ...
          <Field label="Affected device">
            <Select value={form.assetId} onChange={set("assetId")} style={{ width: "100%" }}
              options={[{ value: "", label: "— none / not sure —" },
                ...assetPool.map((d) => ({ value: d.id, label: `${d.name} · ${d.model}` }))]} />
          </Field>
        )}
        <Field label="Impact">
          <Select value={form.impact} onChange={set("impact")} style={{ width: "100%" }}
            options={Object.entries(LEVELS).map(([v, l]) => ({ value: v, label: l.label }))} />
        </Field>
        <Field label="Urgency">
          <Select value={form.urgency} onChange={set("urgency")} style={{ width: "100%" }}
            options={Object.entries(LEVELS).map(([v, l]) => ({ value: v, label: l.label }))} />
        </Field>
        {full && (
          <>
            <Field label="Assign to">
              <Select value={form.assigneeId} onChange={set("assigneeId")} style={{ width: "100%" }}
        ...
        <div style={{ gridColumn: "span 2", display: "flex", alignItems: "center", gap: 10,
          ...
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: ".1em",
            textTransform: "uppercase", color: "var(--text-3)" }}>Computed priority</span>
