...
export function BarChart({ data, maxBars = 8 }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {rows.map((d) => (
        <div key={d.label} style={{ display: "grid", gridTemplateColumns: "92px 1fr 32px", gap: 10, alignItems: "center" }}>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: ".08em",
            textTransform: "uppercase", color: "var(--text-3)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {d.label}
          </span>
          <div style={{ height: 10, background: "var(--surface-2)", borderRadius: "var(--radius-full)", overflow: "hidden" }}>
            <div style={{ width: `${(d.value / max) * 100}%`, height: "100%", borderRadius: "var(--radius-full)",
              background: d.color || "var(--accent)", transition: "width var(--dur-slow) var(--ease-out)" }} />
          </div>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--text-1)", textAlign: "right" }}>{d.value}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "flex-end", gap: 0, height, borderBottom: "1px solid var(--border)" }}>
        {months.map((m, mi) => (
          <div key={m.key} style={{ flex: 1, display: "flex", alignItems: "flex-end", justifyContent: "center", gap: 4, height: "100%" }}>
            {series.map((s) => (
              <div key={s.label} title={`${m.label} · ${s.label}: ${s.values[mi]}`}
                style={{ width: 14, height: `${(s.values[mi] / max) * 92}%`, minHeight: s.values[mi] > 0 ? 3 : 0,
      ...
      <div style={{ display: "flex", marginTop: 6 }}>
        {months.map((m) => (
          <span key={m.key} style={{ flex: 1, textAlign: "center", fontFamily: "var(--font-mono)",
            fontSize: 10, letterSpacing: ".08em", textTransform: "uppercase", color: "var(--text-3)" }}>
            {m.label}
          </span>
        ))}
      </div>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
      <div style={{ position: "relative", flex: "0 0 auto", width: size, height: size }}>
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column",
          ...
          {centerLabel && <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, letterSpacing: ".12em",
            textTransform: "uppercase", color: "var(--text-3)" }}>{centerLabel}</span>}
        </div>
      </div>
